...
// ============================================================================
// MELE VR - Stage 1: VR controller input (integrated from mele-vr-stage1
...
//      DriveAimWithHead() call sites (on-foot + Mako). NO new game-object
...
// hand-written in xr_types.h (values pinned 2026-09-23 against Khronos
// openxr.h release-1.0.34; the bundled openxr_loader.dll is 1.0-generation
// by binary analysis - see STAGE1_CONTROLLER_DESIGN.md section 9). The
// action-state API is xrSyncActions + xrGetActionState* (session-based).
// Init() logs which functions resolved - the first run's [XRINPUT] lines
// confirm the loader generation.
// ============================================================================
...
// Outputs are written only on success (fail-safe for the aim-source swap).
...
// Fill *state (whole XINPUT_STATE, incl. dwPacketNumber = 0) from the current
// frame using cfg (deadzones, right-stick policy). Returns true when a
// controller frame was used - the caller MUST then return ERROR_SUCCESS from
// the XInput hook even if no physical pad is connected (the game sees a
// plugged-in pad).
// The caller increments dwPacketNumber.
